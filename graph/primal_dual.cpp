...
class PrimalDual {
  ...
  public:
    ...
    class EdgePtr {
        ...
        explicit EdgePtr(PrimalDual* p, const usize u, const usize e) : self(p), u(u), e(e) {}
    ...
    EdgePtr add_edge(const usize src, const usize dst, const Flow lower, const Flow upper, const Cost cost) {
        assert(src < size());
        assert(dst < size());
        assert(lower <= upper);
    ...
    void add_supply(const usize u, const Flow f) {
        assert(u < size());
        gap[u] += f;
    }
    ...
    template <class Result = Cost> std::pair<Result, bool> solve_bflow() {
        ...
        Cost farthest;
        const auto dual = [&] {
            over.erase(std::remove_if(over.begin(), over.end(), [&](const usize u) { return gap[u] <= 0; }),
                       over.end());
            lack.erase(std::remove_if(lack.begin(), lack.end(), [&](const usize u) { return gap[u] >= 0; }),
                       lack.end());
    ...
    template <class Result = Cost> std::pair<Flow, Result> flow(const usize src, const usize dst) {
        return flow<Result>(src, dst, std::numeric_limits<Flow>::max());
    }
    template <class Result = Cost>
    std::pair<Flow, Result> flow(const usize src, const usize dst, const Flow flow_limit) {
        assert(src != dst);
