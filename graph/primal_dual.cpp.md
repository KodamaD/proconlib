...
data:
  ...
  bundledCode: "#line 2 \"graph/primal_dual.cpp\"\n#include <algorithm>\n#include\
    ...
    \n\ntemplate <class Flow, class Cost, std::enable_if_t<std::is_integral_v<Flow>\
    \ and std::is_integral_v<Cost>>* = nullptr>\nclass PrimalDual {\n    struct Edge\
    \ {\n        usize dst, rev;\n        Flow flow, cap;\n        Cost cost;\n  \
  ...
  code: "#pragma once\n#include <algorithm>\n#include <cassert>\n#include <type_traits>\n\
    ...
    \ <class Flow, class Cost, std::enable_if_t<std::is_integral_v<Flow> and std::is_integral_v<Cost>>*\
    \ = nullptr>\nclass PrimalDual {\n    struct Edge {\n        usize dst, rev;\n\
